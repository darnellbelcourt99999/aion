...
public final class P2pMgr implements IP2pMgr {
    ...
    private ScheduledExecutorService scheduledWorkers = Executors.newScheduledThreadPool(7);
    ...
    public void run() {
        try {
            ...
            if (p2pLOG.isInfoEnabled()) {
                scheduledWorkers.scheduleWithFixedDelay(() -> printStatus(nodeMgr, selfShortId, receiveMsgQue, p2pLOG, surveyLog), PERIOD_STATUS, PERIOD_STATUS, TimeUnit.SECONDS);
            }
            ...
            scheduledWorkers.scheduleWithFixedDelay(() -> clearTimeoutPeers(nodeMgr), PERIOD_CLEAR, PERIOD_CLEAR, TimeUnit.SECONDS);
            scheduledWorkers.scheduleWithFixedDelay(() -> connectPeers(this, nodeMgr, maxActiveNodes, selector, cachedReqHandshake1, p2pLOG), PERIOD_CONNECT_OUTBOUND, PERIOD_CONNECT_OUTBOUND, TimeUnit.SECONDS);
    ...
    public void send(int nodeId, String nodeDisplayId, final Msg message, Dest peerList) {
        scheduledWorkers.execute(() -> {
            ...
            process(nodeId, nodeDisplayId, message, peerList, System.nanoTime());
    ...
    private static void printStatus(INodeMgr nodeMgr, String selfShortId, BlockingQueue<MsgIn> receiveMsgQue, Logger p2pLOG, Logger surveyLog) {
        ...
        p2pLOG.info(status);
        p2pLOG.debug("receive queue[{}] send queue[{}]", receiveMsgQue.size());

        long duration = System.nanoTime() - startTime;
        surveyLog.debug("TaskStatus: duration = {} ns.", duration);
    }

    private static void clearTimeoutPeers(final INodeMgr nodeMgr) {
